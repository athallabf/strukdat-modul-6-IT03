...
public:
  ...
  // Setter methods
  void setNRP(Data& mhs, long long int NRP) {
    mhs.NRP = NRP;
    }

  void setNama(Data& mhs, const string& nama) {
    mhs.nama = nama;
    }

  void setTugas(Data& mhs, int tugas[]) {
    for (int i = 0; i < 3; i++) {
      mhs.tugas[i] = tugas[i];
      }
    }
  ...
  // Getter methods
  long long int getNRP(const Data& mhs) {
    return mhs.NRP;
    }

  string getNama(const Data& mhs) {
    return mhs.nama;
    }
